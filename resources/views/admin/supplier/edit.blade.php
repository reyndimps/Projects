    ...
    <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="md:flex-1 mt-2 mb:mt-0 md:px-3">
        <div class="mb-4">
          ...
          <input class="w-full shadow-inner p-4 border-0" value="{{old('name', $supplier->name)}}"  type="text" id="name" name="name" placeholder="Fiel Eminedo" required="">
          @error('name')
        ...
        <div class="md:flex mb-4">
          <div class="md:flex-1 md:pr-3">
            ...
            <input class="w-full shadow-inner p-4 border-0" value="{{old('company', $supplier->company)}}" type="text" name="company" placeholder="" required="">
            @error('company')
            <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
          </div>
          <div class="md:flex-1 md:pl-3">
            ...
            <select class="rounded-md w-full py-1.5 p-1 border-2 text-charcoal-darker text-sm" id="country" name="country" required="">
              <option disabled {{ old('country', $supplier->company) ? '' : 'selected' }}>Select Country</option>
          ...
          <div class="md:flex-1 md:pr-3">
            ...
            <div class="md:flex-1 md:pl-3">
              ...
              <select class="rounded-md w-full py-1.5 p-1 border-2 text-charcoal-darker text-sm" id="industry_type" name="industry_type" required="">
                <option disabled {{ old('industry_type', $supplier->industry_type) ? '' : 'selected' }}>Select Industry Type</option>
      ...
      <div class="md:flex mb-8">
        ...
        <div class="md:flex-1 mt-2 mb:mt-0 md:px-3">
          <div class="mb-4">
            ...
            <input class="w-full shadow-inner p-4 border-0" value="{{old('phone', $supplier->phone)}}"  type="tel" name="phone" placeholder="63+" required="">
            @error('phone')
            <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
          </div>
          <div class="mb-4">
            ...
            <input class="w-full shadow-inner p-4 border-0"value="{{old('email', $supplier->email)}}"  type="text" name="email" placeholder="example@gamil.com" required="">
            @error('email')
            <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
          </div>
          <div class="mb-4">
            ...
            <input class="w-full shadow-inner p-4 border-0"value="{{old('address',  $supplier->company)}}"  type="text" name="address" placeholder="" required="">
            @error('address')
            <p class="text-red-600 text-sm">{{ $message }}</p>
            @enderror
          </div>
        ...
        <button type="submit" class="w-full md:w-auto flex items-center justify-end py-3 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
          <svg class="h-4 w-4 mr-1.5 -ml-1" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
